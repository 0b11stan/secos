...
void (*gates[IDT_NR_DESC])();

void register_gate(uint32_t int_nr, void *gate) { gates[int_nr] = gate; }
...
void intr_init() {
   ...
   memset(gates, 0, sizeof(void *) * IDT_NR_DESC);
}
   ...
   if (vector < NR_EXCP)
      ...
      if (gate == 0) {
         debug(
             ...
             "esi     : 0x%x\n"
             "edi     : 0x%x\n",
             ctx->nr.raw, ctx->err.raw, ctx->cs.raw, ctx->eip.raw, ctx->ss.raw,
